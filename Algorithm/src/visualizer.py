"""
File unificato di visualizzazione per l'algoritmo di Piyavski-Shubert.

Funzioni:
    plot_result(tf, result)         — pannello statico completo (3 grafici)
    step_visualizer(tf, result)     — navigatore passo-passo interattivo

Entrambe accettano un TestFunction e un PSResult già calcolato con
store_candidates=True.  Tutte le dipendenze da matplotlib sono locali
a questo modulo: gli altri moduli non ne hanno bisogno.
...
# ── TestFunction (unica definizione, usata anche da main.py) ───────────────

@dataclass
class TestFunction:
    """
    Wrapper leggero che accoppia una funzione al suo dominio e a L.
...
def _draw_function(ax, xs, ys, tf_name: str):
    """Disegna la curva della funzione con area sotto riempita."""
...
def _draw_tents(ax, candidates, tf_L: float, xs, ys):
    """Disegna le tende di Lipschitz per tutti i candidati nella lista."""
    ...
    # limiti y coerenti con la funzione
...
def plot_result(tf: TestFunction, result) -> plt.Figure:
    ...
    ax_pts.set_xlabel("x"); ax_pts.set_ylabel("f(x)")
...
class _StepVisualizer:
    ...
    def __init__(self, tf: TestFunction, result):
        ...
        self._tf     = tf
        self._result = result
        self._n      = len(result.candidates_log)   # numero di snapshot
        self._idx    = 0                             # snapshot corrente
    ...
    def _build_figure(self):
        ...
        # Asse-etichetta: solo testo, niente bordi
    ...
    def _draw(self):
        ...
        _draw_tents(ax, snapshot, tf.L, self._xs, self._ys)

        # Ultimo punto valutato (da history)
        if k > 0:
